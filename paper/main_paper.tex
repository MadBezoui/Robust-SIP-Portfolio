...
Our primary contribution is both theoretical and empirical: We formulate a continuous-state robust CVaR portfolio model in which market states index kernel-weighted empirical return distributions. We establish existence, continuity, convexity, and exact-oracle convergence properties, and implement a finite grid-restricted approximation using adaptive constraint generation. Across 377 consecutive 21-trading-day holding periods, the adaptive method requires only a small subset of candidate state constraints and substantially reduces computation relative to a dense LP on selected windows. The resulting portfolios achieve competitive terminal wealth but do not demonstrate statistically significant Sharpe-ratio superiority or uniformly improved crisis protection relative to simpler benchmarks.
...
\end{equation}
By an elementary covering lemma, if $\widehat{\mathcal{U}}$ has spatial dispersion radius $\rho = \max_{\theta \in \mathcal{U}} \min_{\hat{\theta} \in \widehat{\mathcal{U}}} \|\theta - \hat{\theta}\|$ and $\Phi_\tau(w, \cdot)$ has a uniform Lipschitz constant $L_\Phi$, then the discretization error is bounded by:
\begin{equation}
...
\end{equation}
It then follows that $G(w_k) - v^* \le \varepsilon + L_\Phi \rho$, provided $\text{LB}_k \le v^*$. Because practical values of $L_\Phi \rho$ can be numerically large relative to typical daily CVaR values, this serves strictly as a formal worst-case upper bound rather than a tight continuous-optimality certificate. To ensure the algorithm operates strictly over $\widehat{\mathcal{U}}$, the active subset $U_1$ is initialized with the nearest grid node to the most recent observation $y_T$.
\end{remark}
...
The empirical universe comprises the Kenneth French 30 Industry Portfolios (value-weighted daily returns), representing a broad industry-level representation of the US equity market. Daily asset returns are aligned with:
\begin{itemize}
...
\draw[arrow, dashed, line width=1pt] (-3, 3) -- (10.5, 3) node[right, font=\bfseries\small] {Time (Dec 1994 to June 2026)};
...
\begin{figure}[H]
    ...
    \caption{Out-of-sample cumulative net wealth trajectory across investment strategies from 28 December 1994 to June 2026, accounting for 10 bps transaction costs.}
    \label{fig:wealth}
\end{figure}

\begin{table}[htbp]
\centering
\caption{Comprehensive out-of-sample performance summary (28 December 1994 to June 2026, net of 10 bps transaction costs)}
...
Realized 95\% CVaR (Holding-Period, \%) & 11.38 & 8.28 & 8.14 & 8.05 & 9.45 \\
Realized 99\% CVaR (Holding-Period, \%) & 18.10 & 13.39 & 12.35 & 12.62 & 14.79 \\
...
Average Holding-Period Turnover (\%) & 1.69 & 6.76 & 8.23 & 8.78 & 9.48 \\
...
\midrule
Average Number of Active States & 3.66 & 441 \\
Average Exchange Iterations & 3.66 & N/A \\
