...
\begin{tikzpicture}[
    ...
    dist/.style={rectangle, draw, dashed, minimum width=3.2cm, minimum height=1.5cm, fill=orange!10},
...
\node[above=0.2cm of theta] {\textbf{Continuous State $\theta \in \mathcal{U}$}};
...
\node[dist, right=1.5cm of k2] (cond_dist) {$\mathbb{P}(X | \theta) = \sum_{t=1}^T p_t(\theta) \delta_{x_t}$};
...
\begin{tikzpicture}[
    ...
    box/.style={draw, rectangle, minimum width=8.5cm, minimum height=1cm, align=center},
    arrow/.style={thick, ->, >=stealth}
]
...
\node (step6) [box, right=1.2cm of check] {\textbf{4. TERMINATION}\\Set optimal portfolio $w^* = w_k$.\\Retain active stress set $\mathcal{U}^* = \mathcal{U}_k$.};

\draw [arrow] (step1) -- (step2);
\draw [arrow] (step2) -- (step3);
\draw [arrow] (step3) -- (check);
\draw [arrow] (check.west) -- ++(-1.8,0) |- node[pos=0.25, right] {Gap $> \epsilon$. Set $\mathcal{U}_{k+1} = \mathcal{U}_k \cup \{\theta^*\}$. Increment $k$.} (step2.west);
...
\node[oos, minimum width=1.8cm, right=0cm of train1] (test1) {OOS $T_{\text{hold}} = 21$ days};
\node[right=0.2cm of test1] {Rebalance, record returns};

\node[timeline, minimum width=6cm] (train2) at (1.5, 0.5) {Estimation Window shifted by 21 trading days};
\node[oos, minimum width=1.8cm, right=0cm of train2] (test2) {OOS $T_{\text{hold}} = 21$ days};
\node[right=0.2cm of test2] {Rebalance, record returns};

\node[timeline, minimum width=6cm] (train3) at (3.0, -1) {Estimation Window shifted by $2 \times 21$ trading days};
\node[oos, minimum width=1.8cm, right=0cm of train3] (test3) {OOS $T_{\text{hold}} = 21$ days};
\node[right=0.2cm of test3] {Rebalance, record returns};

\draw[arrow, dashed] (-3, 3) -- (11, 3) node[right] {Time (1995 to 2026)};
...
Figure \ref{fig:frontier} illustrates the in-sample efficient frontier, while Figure \ref{fig:kernel} displays the continuous empirical density map of the state variables. The joint support of VIX and market drawdown exhibits strong non-linear clustering, demonstrating that continuous kernel weighting captures intermediate stress conditions that discrete four-quadrant models fail to represent.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\textwidth]{../figures/bootstrap_plot.pdf}
    \caption{Studentized circular block-bootstrap distribution of the out-of-sample Sharpe ratio difference between Robust SIP and Nominal CVaR.}
    \label{fig:bootstrap}
\end{figure}
...
The point estimate difference in annualized Sharpe ratios between Robust SIP and Nominal CVaR is $\Delta \text{SR} = -0.0043$, with a 95\% confidence interval spanning $[-0.0450, 0.0437]$ and a $p$-value of $p = 0.822$. As shown in Figure \ref{fig:bootstrap}, the bootstrap distribution is symmetric and centered near zero. 
