...
\begin{enumerate}
    ...
    \item \textbf{Rigorous Statistical and Computational Diagnostics}: We report 14 distinct performance metrics gross and net of transaction costs, evaluate crisis-period performance, analyze transaction cost sensitivity, evaluate computational convergence against dense grid solutions, and conduct paired circular moving-block bootstrap hypothesis testing on Sharpe ratio differences following \citet{ledoit2008robust}.
\end{enumerate}
...
Let $\hat{\mu}_{(1)} \ge \hat{\mu}_{(2)} \ge \dots \ge \hat{\mu}_{(N)}$ denote the sorted components of $\hat{\mu}$. The capped simplex $\{w \in \mathbb{R}^N : \mathbf{1}^\top w = 1, 0 \le w_i \le \bar{w}\}$ is non-empty if and only if $N \bar{w} \ge 1$. Furthermore, the set $W$ is non-empty if and only if $N \bar{w} \ge 1$ and
\begin{equation}
    \mu_{\text{target}} \le \mu_{\max} = \bar{w} \sum_{i=1}^{q} \hat{\mu}_{(i)} + r \hat{\mu}_{(q+1)}
\end{equation}
where $q = \lfloor 1/\bar{w} \rfloor$ and $r = 1 - q\bar{w}$. When non-empty, $W$ is a compact convex subset of $\mathbb{R}^N$.
\end{proposition}
...
\end{equation}
Termination when $\widehat{G}_k - \text{LB}_k \le \epsilon$ confirms numerical convergence of the exchange algorithm to the finite $21 \times 21$ robust problem, and guarantees that $w_k$ is $(\epsilon + L_\Phi \rho)$-feasible for the continuous SIP.
\end{remark}
...
\begin{enumerate}
    \item \textbf{Return Generation}: Robust SIP achieves an annualized mean return of 11.28\%, outperforming Nominal CVaR (10.76\%) and MinVar (10.65\%), and ranking second to naive $1/N$ (12.24\%) and Finite-Regime CVaR (11.15\%). The $1/N$ portfolio benefits from broad cross-sectional diversification and full exposure to the long-run equity market risk premium over the 31-year evaluation period.
    \item \textbf{Risk-Adjusted Ratios}: Finite-Regime CVaR achieves the highest Sharpe ratio (0.901), followed by Nominal CVaR (0.880) and MinVar (0.868). Robust SIP records a Sharpe ratio of 0.809. This reflects the higher annualized volatility (13.95\%) induced by state-dependent conditioning, which tilts allocations toward sectors with higher expected returns but also higher variance.
...
(Mar 2000 to Oct 2002) & MinVar & +1.36 & -20.66 \\
...
(Oct 2007 to Mar 2009) & MinVar & -34.67 & -40.18 \\
...
(Feb 2020 to Apr 2020) & MinVar & -12.25 & -15.29 \\
...
(Jan 2022 to Dec 2022) & MinVar & -6.47 & -18.12 \\
...
\begin{figure}[htbp]
    ...
    \begin{minipage}{0.48\textwidth}
        ...
        \caption{Minimum Variance asset allocations over time across the 30 industry portfolios (max 15\% cap).}
        \label{fig:weights_mv}
    \end{minipage}
\end{figure}

Figures \ref{fig:weights_rob} and \ref{fig:weights_mv} compare the dynamic allocation weights of Robust SIP and Minimum Variance. Robust SIP maintains allocations across 7 to 10 active industry sectors ($N_{\text{eff}} = 7.36$), reallocating capital across cyclical and defensive sectors as historical window dynamics evolve. Conversely, the Minimum Variance portfolio remains concentrated in low-beta industries ($N_{\text{eff}} = 8.15$), sacrificing participation in broad equity market rallies.
...
Figure \ref{fig:turnover} confirms that the turnover distribution for Robust SIP has a median near 9.5\%, reflecting the active rebalancing induced by continuous state conditioning. MinVar exhibits lower median turnover (6.8\%), while $1/N$ has the lowest (1.7\%) due to its static equal-weight target.
...
MinVar & 0.874 & 0.871 & 0.868 & 0.861 & 0.842 \\
...
MinVar & 22.61 & 22.33 & 22.05 & 21.50 & 19.94 \\
...
Figure \ref{fig:bounds} tracks the convergence of the Master LP Lower Bound ($\text{LB}_k$) and the Grid Separation Worst-Case Value ($\widehat{G}_k$). The shaded green area denotes the grid-restricted exchange gap ($\widehat{G}_k - \text{LB}_k$). Starting from an initial gap of 2.957\% at iteration $k=1$, the master lower bound increases monotonically from 1.701\% as active stress constraints are appended, while the oracle worst-case value contracts from 4.658\%. Within 6 exchange iterations, the gap collapses below $10^{-4}$, confirming numerical convergence of the grid-restricted exchange algorithm to the $21 \times 21$ finite robust problem.
...
\midrule
\textbf{Effective Sample Size of Active States} & & \\
Minimum ESS & \multicolumn{2}{c}{2.36} \\
5th Percentile ESS & \multicolumn{2}{c}{15.83} \\
Median ESS & \multicolumn{2}{c}{71.36} \\
Mean ESS & \multicolumn{2}{c}{83.42} \\
Maximum ESS & \multicolumn{2}{c}{280.19} \\
Average Tail ESS ($\tau \times \text{ESS}$) & \multicolumn{2}{c}{4.17} \\
\bottomrule
\end{tabular}
}
\end{table}
...
Several diagnostics in Table \ref{tab:computational} warrant careful interpretation:
\begin{enumerate}
...
\begin{figure}[H]
    ...
    \caption{Paired circular moving-block bootstrap distribution ($B = 5{,}000$ replications, block length $b = 12$ months) for the out-of-sample Sharpe ratio difference between Robust SIP and Nominal CVaR.}
    \label{fig:bootstrap}
\end{figure}
...
vs.\ MinVar & $-0.0586$ & 0.0681 & $[-0.202, \, 0.066]$ & 0.384 \\
vs.\ Finite-Regime & $-0.0917$ & 0.0613 & $[-0.215, \, 0.026]$ & 0.132 \\
\bottomrule
\end{tabular}
\end{table}

Against all four benchmarks, the two-sided $p$-values exceed 0.05 ($p = 0.281$ vs. Nominal CVaR, $p = 0.328$ vs. $1/N$, $p = 0.384$ vs. MinVar, and $p = 0.132$ vs. Finite-Regime). The null hypothesis of equal Sharpe ratios is not rejected at the 5\% or 10\% significance levels. As shown in Figure \ref{fig:bootstrap}, the bootstrap distribution of $\Delta \text{SR}$ relative to Nominal CVaR is approximately symmetric and centered at $-0.071$, with the null value $\Delta \text{SR} = 0$ comfortably contained within the 95\% confidence interval.
...
The proposed state-robust CVaR framework provides a principled and interpretable method for incorporating continuous market covariates into portfolio optimization without imposing artificial discrete regime boundaries. In the reported 31-year empirical experiment on US industry portfolios, the continuous-state strategy achieved greater terminal wealth (\$25.06) than Minimum Variance (\$22.05) and nominal CVaR (\$22.92), but lower wealth than naive $1/N$ (\$29.25) and finite-regime CVaR (\$25.68). It also exhibited lower Sharpe ratios and deeper maximum drawdowns than the nominal and finite-regime CVaR benchmarks, and paired moving-block bootstrap testing confirmed that Sharpe ratio differences are not statistically significant at conventional levels. Thus, the empirical findings do not establish superior risk-adjusted or crisis performance.
