...
\begin{document}
\maketitle
\begin{NoHyper}\def\thefootnote{*}\footnotetext{Corresponding author. Email: madani.bezoui@cesi.fr}\end{NoHyper}
...
\subsection{Robust and distributionally robust portfolio optimization}
...
\section{Continuous market-state modeling}
...
\begin{enumerate}
    \item \textbf{Implied Market Volatility ($V_t$)}: The Chicago Board Options Exchange (CBOE) Volatility Index (VIX), which reflects forward-looking market uncertainty and implied option volatility.
    \item \textbf{Market Drawdown ($D_t$)}: The peak-to-trough decline of the broad equity market index over a trailing quarterly lookback window of $L = 63$ trading days, defined as:
    \begin{equation}
...
\subsection{Definition of the continuous state space}
...
\subsection{Kernel-weighted conditional empirical distributions}
...
\begin{equation}
    \mathbb{P}(X | \theta) = \sum_{t=1}^{T} p_t(\theta) \delta_{x_t}
\end{equation}
where $\delta_{x_t}$ denotes the Dirac delta mass at historical return vector $x_t$.
...
\subsection{Effective sample size diagnostics}
When evaluating conditional risk in regions of the state space with low historical data density, the kernel estimator concentrates probability mass on a small subset of observations. To quantify local data richness, we compute the Effective Sample Size (ESS) for any state $\theta \in \mathcal{U}$:
\begin{equation}
...
\section{Continuous-state robust CVaR optimization}
...
\subsection{Conditional value-at-risk formulation}
For a given portfolio allocation $w \in W$ and a fixed state $\theta \in \mathcal{U}$, we measure portfolio downside risk using the Conditional Value-at-Risk at tail probability level $\tau \in (0, 1)$, corresponding to confidence level $\alpha = 1 - \tau$. We set $\tau = 0.05$ ($\alpha = 0.95$) throughout the empirical analysis. Following the variational representation of \citet{rockafellar2000optimization}, the conditional CVaR evaluated at state $\theta$ is:
\begin{equation}
...
\subsection{The semi-infinite programming formulation}
...
\end{align}
Because $\mathcal{U}$ is an uncountably infinite set, the problem contains an infinite family of continuous convex constraints $\Phi_\tau(w, \theta) \le \eta$.

\subsection{Theoretical properties: existence, continuity, and convexity}
...
Under the assumptions of Proposition 1, the robust objective function:
\begin{equation}
...
\section{The adaptive exchange algorithm}
...
\subsection{The finite master linear program}
...
\subsection{The continuous separation oracle}
...
\begin{equation}
    \theta^* = \arg\max_{\theta \in \mathcal{U}} \Phi_\tau(w_k, \theta)
\end{equation}
and records the corresponding upper bound $\text{UB}_k = \Phi_\tau(w_k, \theta^*)$.

The conditional CVaR evaluation $\Phi_\tau(w_k, \theta)$ requires solving a one-dimensional convex optimization problem over $z \in \mathbb{R}$. For a fixed state $\theta$, the objective $z \mapsto z + \frac{1}{\tau} \sum_{t=1}^T p_t(\theta) [-x_t^\top w_k - z]_+$ is piecewise linear and convex. Its global minimum is attained at the weighted empirical quantile $z^*(\theta) = \text{VaR}_\tau(w_k, \theta)$, which can be computed in $O(T \log T)$ time by sorting the portfolio losses $l_t = -x_t^\top w_k$ and finding the smallest loss whose cumulative kernel weight reaches $\tau$.
...
\section{Experimental design and institutional backtesting protocol}
...
\subsection{Asset universe and macroeconomic data}
...
\subsection{Rolling-window backtest protocol}
...
\begin{enumerate}
    \item The target expected return $\mu_{\text{target}}$ is dynamically set at each rebalancing date to the cross-sectional median of historical asset means: $\mu_{\text{target}} = \text{median}(\hat{\mu})$. This guarantees that $\mu_{\text{target}}$ is feasible in every window while avoiding unrealistically aggressive return hurdles.
    \item The identical target return constraint $\hat{\mu}^\top w \ge \mu_{\text{target}}$ is enforced on the Robust SIP model, the Nominal CVaR model, and the Finite-Regime CVaR model.
\end{enumerate}

\subsection{Benchmark strategies}
We compare the continuous-state Robust SIP against four established benchmark strategies:
\begin{enumerate}
    \item \textbf{Equal Weight (1/N)}: Assigns equal weight $w_i = 1/N$ to all 30 industry portfolios. This benchmark is free of parameter estimation error \citep{demiguel2009optimal}.
    \item \textbf{Global Minimum Variance (MinVar)}: Solves $\min_{w \in W_0} w^\top \hat{\Sigma} w$, where $\hat{\Sigma}$ is the sample covariance matrix and $w \in W_0 = \{w \ge 0, \sum w_i = 1\}$. It ignores expected returns to minimize variance.
    \item \textbf{Nominal CVaR}: Minimizes standard unconditional CVaR at $\tau = 0.05$ over the full training sample subject to $\hat{\mu}^\top w \ge \mu_{\text{target}}$, ignoring market states.
    \item \textbf{Finite-Regime CVaR}: Partitions the state space into 4 discrete regimes based on median sample splits of VIX and drawdown (low VIX/low DD, high VIX/low DD, low VIX/high DD, high VIX/high DD). It optimizes portfolio weights against the worst-case regime CVaR among the 4 discrete regimes.
\end{enumerate}

\subsection{Turnover and transaction cost modeling}
...
\begin{equation}
    \text{TO}_t = \frac{1}{2} \sum_{i=1}^N \left| w_{i, t} - w_{i, t}^{\text{pretrade}} \right|
\end{equation}
A proportional transaction cost penalty $c = 10$ basis points (0.10\%) is deducted from gross returns at each rebalancing date:
\begin{equation}
    r_{t}^{\text{net}} = r_{t}^{\text{gross}} - c \cdot \text{TO}_t
\end{equation}
We also evaluate sensitivity to alternative transaction cost levels $c \in \{0, 10, 25, 50\}$ bps.

\subsection{Historical crisis periods}
To evaluate downside risk immunization during systemic shocks, we define four historical crisis windows ex ante:
\begin{itemize}
    \item \textbf{Dot-Com Crash}: March 2000 to October 2002 (technology bubble collapse).
    \item \textbf{Global Financial Crisis (GFC)}: October 2007 to March 2009 (subprime mortgage crisis).
    \item \textbf{COVID-19 Shock}: February 2020 to April 2020 (global pandemic market crash).
    \item \textbf{Inflation and Rate Shock}: January 2022 to December 2022 (rapid monetary tightening).
\end{itemize}

\section{Empirical results and financial analysis}
...
\subsection{Long-term cumulative wealth and risk-adjusted returns}
...
\subsection{Drawdown profiles and downside protection}
...
\subsection{Portfolio composition and asset concentration}
...
\subsection{Computational efficiency and active state parsimony}
...
\begin{figure}[H]
    ...
    \caption{Master LP lower bound and oracle upper bound convergence across iterations.}
    \label{fig:bounds}
\end{figure}

Figure \ref{fig:bounds} tracks the monotonic convergence of the master lower bound ($\text{LB}_k$) and the oracle upper bound ($\text{UB}_k$). Within 3 to 5 exchange iterations, the optimality gap collapses below the convergence tolerance $\epsilon = 10^{-4}$, confirming rapid numerical convergence.
...
\subsection{Statistical inference via studentized block-bootstrap}
To determine whether the observed performance differences are statistically distinguishable from sample noise, we implement the studentized circular block-bootstrap test for Sharpe ratio differences following \citet{ledoit2008robust}. Because monthly financial returns exhibit temporal dependence and volatility clustering, standard i.i.d. $t$-tests produce severely understated standard errors.

We resample monthly strategy return pairs using a block size of $b = 12$ months over $B = 2000$ bootstrap replications. Table \ref{tab:bootstrap} reports the point estimate, two-sided 95\% confidence interval, and empirical $p$-value for the Sharpe ratio difference $\Delta \text{SR} = \text{SR}_{\text{Robust}} - \text{SR}_{\text{Nominal}}$.
...
\begin{enumerate}
    \item \textbf{Curse of Dimensionality in State Variables}: While the two-dimensional state space (VIX and Drawdown) effectively captures market volatility and price distress, extending the state vector $y_t$ to include interest rate spreads, credit default spreads, and liquidity metrics would dilute kernel weights in high dimensions. Future research should explore dimension-reduction techniques, such as autoencoder latent spaces or conditional generative neural networks, to model high-dimensional state spaces.
    \item \textbf{Oracle Solvers for High Dimensions}: In two dimensions, global grid evaluation provides an efficient separation oracle. In higher-dimensional state spaces, global branch-and-bound algorithms or exact bilinear programming reformulations could provide certified optimality bounds without heuristic approximations.
    \item \textbf{Transaction Cost Regularization}: While monthly turnover was modest (7.03\%), integrating explicit $L_1$ turnover regularization directly into the master linear program would further suppress transaction costs in institutional settings with higher trading frictions.
\end{enumerate}
