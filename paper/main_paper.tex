...
At iteration $k \ge 1$, the master problem optimizes portfolio weights $w$ and the robust risk level $\eta$ subject to a finite active subset of market states $\widehat{\mathcal{U}}_k = \{\theta^{(1)}, \dots, \theta^{(m_k)}\} \subseteq \widehat{\mathcal{U}} \subset \mathcal{U}$. For each active state $\theta^{(j)} \in \mathcal{U}_k$, we introduce a state-specific VaR variable $z_{\theta^{(j)}} \in \mathbb{R}$ and scenario excess loss variables $u_{t, \theta^{(j)}} \ge 0$. The master problem is the following finite Linear Program (LP):
\begin{align}
...
\begin{proof}
Because $\widehat{\mathcal{U}}_k \subseteq \widehat{\mathcal{U}} \subset \mathcal{U}_{k+1} \subset \mathcal{U}$, the feasible region of the master LP shrinks monotonically, implying $\text{LB}_k \le \text{LB}_{k+1} \le v^\star$. For Item 2, $G(w_k) = \sup_{\theta \in \mathcal{U}} \Phi_\tau(w_k, \theta) \ge \min_{w \in W} \sup_{\theta \in \mathcal{U}} \Phi_\tau(w, \theta) = v^\star$.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\textwidth]{../results/wealth_plot.pdf}
...
\begin{enumerate}
    \item \textbf{Return Generation}: Robust SIP achieves an annualized mean return of 11.28\%, achieving a higher point estimate than Nominal CVaR (10.76\%) and TC-MinVar (10.75\%), and ranking second to naive $1/N$ (12.24\%) and Finite-Regime CVaR (11.15\%). The $1/N$ portfolio benefits from broad cross-sectional diversification and full exposure to the long-run equity market risk premium over the 31-year evaluation period.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\textwidth]{../results/drawdown_plot.pdf}
...
\begin{figure}[htbp]
    ...
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../results/weights_rob_plot.pdf}
    ...
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../results/weights_mv_plot.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\textwidth]{../results/turnover_plot.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\textwidth]{../results/cumulative_tc_plot.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\textwidth]{../results/active_states_plot.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\textwidth]{../results/bounds_plot.pdf}
...
\begin{enumerate}
    ...
    \item \textbf{Active-State Support}: Active-state support: The minimum, across rolling windows, of the within-window mean ESS over active states is 2.18, indicating that some windows contain collectively low-support active states. The reported value $\tau\times \operatorname{ESS}$ is only a descriptive tail-support proxy and is not a formal tail effective sample size.\end{enumerate}


\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.88\textwidth]{../results/ess_history_plot.pdf}
...
\begin{figure}[htbp]
    ...
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../results/frontier_plot.pdf}
    ...
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../results/kernel_map_plot.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\textwidth]{../results/bootstrap_plot.pdf}
...
To ensure scientific transparency and enable independent verification of all empirical results, the entire computational framework is open-source under an MIT License. The official repository is hosted at \url{https://github.com/MadBezoui/Robust-SIP-Portfolio}. The exact code and results corresponding to this manuscript are available under the immutable GitHub tag v1.0.6. A matching Zenodo archive will be cited upon availability.

The repository provides the complete pipeline structured into four main components:
\begin{enumerate}
    \item \textbf{Optimization Engine} (\texttt{code/Robust SIP.jl}): Implements the grid-restricted adaptive constraint-generation algorithm, the HiGHS master-LP solver, the full-grid separation oracle, and the benchmark solvers.
...
\textbf{Data Availability:} The Kenneth French 30 Industry Portfolio returns and CBOE VIX observations used in this study were obtained from their publicly accessible source pages. Source URLs, retrieval information, preprocessing code, and the aligned research dataset are documented in the accompanying repository, subject to the original providers' terms of use. The code and pre-processed data used to generate the results are available in the GitHub repository: \url{https://github.com/MadBezoui/Robust-SIP-Portfolio}.
