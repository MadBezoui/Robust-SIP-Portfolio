...
This repository contains the complete reproducible codebase, historical datasets, numerical tables, and generated publication figures for the research paper: **"From market states to robust portfolios: adaptive semi-infinite CVaR optimization with real financial data"** by Madani Bezoui and Thiziri Sifaoui.
...
- `code/`: Contains the complete Julia and Python source code.
  ...
  - `main_exp.jl`: The rolling-window backtest pipeline over 30 years comparing Robust SIP against 1/N, MinVar, Nominal CVaR, and Finite-Regime CVaR, including the studentized circular block-bootstrap test and sensitivity analyses.
...
- `figures/`: Contains the generated analytical figures (PDF) and summary numerical tables (CSV).
  ...
  - `bootstrap_inference.csv`: Studentized circular block-bootstrap Sharpe ratio difference inference.
...
The script will execute the rolling-window backtest, perform the exchange optimizations, run the Ledoit-Wolf studentized bootstrap, and export all figures and CSV tables directly into `figures/`.
