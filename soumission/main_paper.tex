...
\begin{tabular}{>{\raggedright\arraybackslash}p{0.22\linewidth}>{\raggedright\arraybackslash}p{0.2\linewidth}>{\raggedright\arraybackslash}p{0.26\linewidth}>{\raggedright\arraybackslash}p{0.26\linewidth}}
\toprule
...
\begin{tabular}{lrrrr}
\toprule
\textbf{Grid Resolution} & \textbf{Runtime (s)} & \textbf{Active States} & \textbf{In-Sample Worst-Case CVaR$_{95}$ (\%)} & \textbf{Dist to Base} \\
\midrule
...
\appendix
\section{Derivation of the Global Lipschitz Constant}\label{app:lipschitz}
...
\begin{equation}
    \Phi_\tau(w, \theta) = \min_{z \in [-M_L, M_L]} F(w, z, \theta), \quad \text{where} \quad F(w, z, \theta) = z + \frac{1}{\tau} \sum_{t=1}^T p_t(\theta) [\ell_t(w) - z]_+
\end{equation}
...
\end{equation}
Because any VaR minimizer $z$ lies in $[-M_L, M_L]$ and $\ell_t(w) \in [-M_L, M_L]$, the term $[\ell_t(w) - z]_+$ is uniformly bounded by $2M_L$. A standard calculation for Nadaraya--Watson weights with a Gaussian kernel $K_H(u) \propto \exp(-\frac{1}{2} u^\top H^{-1} u)$ shows that the Jacobian $\nabla_\theta p_t(\theta)$ involves terms bounded by the inverse bandwidth matrix. Specifically, using the global bound $R = \sup_{\theta, t} \|y_{t-1} - \theta\|$, the gradient norm is conservatively bounded by $\|\nabla_\theta p_t(\theta)\| \le \frac{2 R}{\lambda_{\min}(H)}$. Summing these bounds across the $T$ normalized weights yields the global Lipschitz constant:
\begin{equation}
    L_\Phi := \frac{4 R M_L}{\tau \lambda_{\min}(H)}
\end{equation}
In the numerical implementation, we use the Euclidean norm. For the normalized log-VIX and drawdown domain, $R \approx 100$, and with $H$ determined by Scott's rule, $\lambda_{\min}(H) \approx 0.05$, yielding $L_\Phi$ on the order of $10^2$ to $10^3$, which motivates the empirical local search strategy presented in the main text.
