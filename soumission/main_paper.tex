...
\begin{document}
% The journal caps artwork at 119 mm; the smallcondensed type area is
% 122 mm wide, so figures are set to the cap rather than to \textwidth.
\newlength{\figwidth}\setlength{\figwidth}{119mm}
\sloppy
% Tighten the vertical space around floats. The document carries 38 floats, so
% the default separations dominate the page budget; these values are still
% comfortably above the point where a float would touch its surrounding text.
\setlength{\textfloatsep}{10pt plus 2pt minus 2pt}
\setlength{\floatsep}{8pt plus 2pt minus 2pt}
\setlength{\intextsep}{8pt plus 2pt minus 2pt}
\setlength{\abovecaptionskip}{4pt}
\setlength{\belowcaptionskip}{2pt}
% Let LaTeX fill pages more fully with floats before deferring them.
...
\institute{Madani Bezoui (corresponding author) \at
              ...
              \email{mbezoui@cesi.fr} --- \orcidlink{0000-0002-8342-7039}~0000-0002-8342-7039
           \and
           Thiziri Sifaoui \at
              ...
              LAROMAD, Faculty of Sciences, UMMTO, Tizi Ouzou, Algeria --- \orcidlink{0009-0007-6884-0611}~0009-0007-6884-0611
           \and
           Ahcene Bounceur \at
              College of Computing and Informatics, University of Sharjah, United Arab Emirates --- \email{abounceur@sharjah.ac.ae} --- \orcidlink{0000-0002-0043-7742}~0000-0002-0043-7742
}
...
\begin{abstract}
Standard robust portfolio optimization models represent market conditions through discrete regimes or unconditional ambiguity sets. We instead formulate a continuous-state robust portfolio model that minimizes Conditional Value-at-Risk (CVaR) uniformly over a continuously indexed family of kernel-weighted empirical conditional return distributions, with states indexed by the logarithm of the CBOE Volatility Index (log-VIX) and market drawdown. We establish well-posedness, continuity and convexity of the continuous formulation and a Lipschitz bound on the grid-discretization error. For computation we employ a grid-restricted separation oracle and adaptive constraint generation. Across 40 rolling windows, the master problem retains only 3.3--3.4 active state blocks on average as the candidate grid grows from 121 to 961 states, while exchange runtime grows substantially more slowly than the dense extensive-form LP. In a rolling out-of-sample study of US industry portfolios, 1994--2026, the strategy achieves competitive long-run wealth, but a paired circular moving-block percentile bootstrap provides no evidence of a statistically significant Sharpe-ratio advantage over nominal CVaR. The findings highlight the importance of data support in tail regions and the tractability of a grid-restricted approximation to continuous-state robustification.
...
We emphasize that mathematical well-posedness and statistical reliability are
distinct properties. The strict positivity of the Gaussian kernel ensures that
$\widehat P_\theta$ is defined for every $\theta \in \mathcal{U}$, but it does
not imply that every conditional distribution is estimated with comparable
precision. In low-support regions the conditional empirical measure may
effectively rest on a small number of historical observations, so a state can be
perfectly well defined and still be poorly estimated. We therefore use the
effective sample size as a diagnostic of local estimation reliability, and study
ESS-restricted state domains explicitly in Section \ref{sec:sensitivity}.
...
The optimization objective is based on one-day conditional returns, whereas
portfolio weights are held for 21 trading days. This mismatch is intentional.
Daily observations provide a substantially larger effective sample for estimating
state-dependent tail distributions, since the conditional CVaR at a given state
is estimated from kernel-weighted daily losses; aggregating to the holding
frequency before estimation would divide the usable sample by roughly twenty and
leave the tail of each conditional distribution determined by a handful of
observations. Monthly rebalancing, on the other hand, keeps turnover and
transaction costs at implementable levels. The model therefore uses
high-frequency information for estimation and lower-frequency implementation for
the portfolio decision, and all reported performance statistics are computed
from the realized 21-day holding-period returns.
...
\begin{table}[!htbp]
\centering
\caption{Crisis-period performance. Crisis intervals are defined ex ante by the dates shown and applied identically to all strategies}
\label{tab:crisis}
\resizebox{0.88\textwidth}{!}{
\begin{tabular}{lllrr}
\toprule
\textbf{Crisis period} & \textbf{Window} & \textbf{Strategy} & \textbf{Return (\%)} & \textbf{Max DD (\%)} \\
\midrule
DotCom & 1 Mar 2000--31 Oct 2002 & 1/N & $-4.75$ & $-24.41$ \\
DotCom & 1 Mar 2000--31 Oct 2002 & TC-MinVar & 1.36 & $-20.66$ \\
DotCom & 1 Mar 2000--31 Oct 2002 & Nominal CVaR & 5.86 & $-20.66$ \\
DotCom & 1 Mar 2000--31 Oct 2002 & Finite-Regime CVaR & 17.26 & $-20.33$ \\
DotCom & 1 Mar 2000--31 Oct 2002 & Robust SIP & 4.25 & $-24.95$ \\
GFC & 1 Oct 2007--31 Mar 2009 & 1/N & $-53.56$ & $-55.79$ \\
GFC & 1 Oct 2007--31 Mar 2009 & TC-MinVar & $-34.67$ & $-40.18$ \\
GFC & 1 Oct 2007--31 Mar 2009 & Nominal CVaR & $-31.13$ & $-35.76$ \\
GFC & 1 Oct 2007--31 Mar 2009 & Finite-Regime CVaR & $-30.91$ & $-36.76$ \\
GFC & 1 Oct 2007--31 Mar 2009 & Robust SIP & $-36.49$ & $-41.69$ \\
COVID & 1 Feb 2020--30 Apr 2020 & 1/N & $-20.18$ & $-20.64$ \\
COVID & 1 Feb 2020--30 Apr 2020 & TC-MinVar & $-12.25$ & $-15.29$ \\
COVID & 1 Feb 2020--30 Apr 2020 & Nominal CVaR & $-12.05$ & $-15.42$ \\
COVID & 1 Feb 2020--30 Apr 2020 & Finite-Regime CVaR & $-11.67$ & $-15.15$ \\
COVID & 1 Feb 2020--30 Apr 2020 & Robust SIP & $-17.44$ & $-18.99$ \\
Inflation & 1 Jan 2022--31 Dec 2022 & 1/N & $-6.05$ & $-17.24$ \\
Inflation & 1 Jan 2022--31 Dec 2022 & TC-MinVar & $-6.47$ & $-18.12$ \\
Inflation & 1 Jan 2022--31 Dec 2022 & Nominal CVaR & $-4.83$ & $-17.92$ \\
Inflation & 1 Jan 2022--31 Dec 2022 & Finite-Regime CVaR & $-7.66$ & $-18.88$ \\
Inflation & 1 Jan 2022--31 Dec 2022 & Robust SIP & $-8.59$ & $-19.86$ \\
\bottomrule
\end{tabular}
}
\end{table}
...
\subsection{What the empirical evidence does and does not support}\label{sec:empirical_message}
It is worth stating plainly what Tables \ref{tab:performance} and \ref{tab:tc_sensitivity} do not show. Robust SIP does not record the highest Sharpe ratio: at zero transaction cost it reaches 0.809, behind Finite-Regime CVaR (0.901), Nominal CVaR (0.880) and TC-MinVar (0.875), and Section \ref{sec:computational} will show that differences of this magnitude cannot be distinguished from zero by a block bootstrap. Nor does it dominate the CVaR benchmarks during the principal crisis episodes. Any reading of this study as a demonstration that continuous-state robustification improves risk-adjusted performance would misstate the evidence. Section \ref{sec:target} shows that this figure is itself sensitive to the return-target convention, rising to 0.899 within the Robust SIP specification under a tighter target. That experiment holds the benchmarks at their own baseline target and therefore does not license a cross-strategy comparison at the alternative levels; it establishes only that the return-target convention accounts for a material part of the performance profile reported here.

The empirical contribution lies elsewhere, and the relevant comparison is not against the unconditional benchmarks but against the alternative way of using the same state information. Conditioning the return distribution directly on the observed state, without a robust layer, turns over 35.7\% of the portfolio per rebalancing and ends with \$17.82, against 9.48\% and \$25.04 for the robust formulation (Section \ref{sec:referee_benchmarks}); Section \ref{sec:shrinkage} shows that shrinking the conditional law towards the unconditional one does not recover the difference at any intermediate degree of shrinkage. Robustness over a state domain therefore acts as a regularization mechanism against excessive responsiveness to short-lived movements in the conditioning variables: it requires satisfactory tail-risk performance over an entire set of plausible states rather than tracking the one state currently observed, and it delivers state-awareness at close to unconditional trading cost. That, together with the computational sparsity documented in Section \ref{sec:scalability}, is what the experiment supports.
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\figwidth]{Fig7.pdf}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\figwidth]{Fig8.pdf}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\figwidth]{Fig9.pdf}
    \caption{Evolution of the average Effective Sample Size}
    \label{fig:ess_history}
\end{figure}

\subsection{Scalability in the grid dimension}\label{sec:scalability}
Table \ref{tab:computational} compares the two methods on a single window, which is a weak basis for a computational claim. We therefore repeated the comparison on 40 rolling windows, chosen to span the sample evenly while over-representing the highest log-VIX and deepest drawdown states, and solved every window twice: once with the extensive-form dense LP over the full candidate grid, and once with the adaptive exchange algorithm. Both solvers received identical data, identical estimation settings and identical grids. Table \ref{tab:scalability} reports the outcome for three grid resolutions.

\begin{table}[!htbp]
\centering
\caption{Matched dense-LP and exchange-algorithm comparison over 40 rolling windows per grid resolution}
\label{tab:scalability}
\footnotesize
\begin{tabular}{lrrrrrrr}
\toprule
\textbf{Grid} & \textbf{States} & \makecell[r]{\textbf{Dense}\\\textbf{LP (s)}} & \makecell[r]{\textbf{Exch.}\\\textbf{(s)}} & \makecell[r]{\textbf{Speed-}\\\textbf{up}} & \makecell[r]{\textbf{Active}\\\textbf{blocks}} & \makecell[r]{\textbf{Max}\\\textbf{gap}} & \makecell[r]{\textbf{Mean}\\$\boldsymbol{\ell_1}$} \\
\midrule
$11\times11$ & 121 & 6.16 & 0.235 & 36.2 & 3.33 & $7.5\times10^{-7}$ & 0.00001 \\
$21\times21$ & 441 & 27.06 & 0.316 & 110.3 & 3.27 & $1.2\times10^{-5}$ & 0.00018 \\
$31\times31$ & 961 & 105.35 & 0.813 & 242.1 & 3.42 & $2.0\times10^{-5}$ & 0.00017 \\
\bottomrule
\end{tabular}
\end{table}

Three observations follow. First, the two methods agree: the largest absolute difference between the dense optimum and the exchange value over all 120 solves is $2.0\times10^{-5}$, an order of magnitude below the exchange tolerance of $10^{-4}$, and the mean $\ell_1$ distance between the two allocations never exceeds 0.0002. The exchange algorithm is therefore solving the same problem, not an easier one. Second, the speedup is not an artifact of one instance: the paired per-window speedup is at least 11.3 on every one of the 120 solves, and its median rises from 33.1 to 234.3 as the grid is refined. Third, and most importantly, the number of active state blocks does not grow with the grid. As the candidate set expands by a factor of 7.9, from 121 to 961 states, the mean number of blocks retained by the master problem moves only between 3.27 and 3.42, and never exceeds 6 in any window. Dense solution time grows by a factor of 17.1 over the same refinement while exchange time grows by a factor of 3.5.

The computational contribution is therefore not the speedup measured on one benchmark instance, but this stability: the binding subset of the state space stays small as the discretization is refined, so the cost of the master problem is governed by the number of states that actually bind rather than by the resolution of the candidate grid. This is the property that makes a fine discretization of a continuous state space affordable, and it is what a purely dense formulation cannot offer. We did not extend the matched comparison beyond $31\times31$ because the extensive-form dense LP becomes the binding constraint, not the exchange algorithm; the single-window result in Table \ref{tab:computational}, where the dense $41\times41$ LP returned no primal solution within 600 seconds, illustrates the point.


\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\figwidth]{Fig10.pdf}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\figwidth]{Fig11.pdf}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\figwidth]{Fig12.pdf}
...
Against all four benchmarks, the bootstrap $p$-values exceed 0.05 ($p = 0.281$ vs. Nominal CVaR, $p = 0.330$ vs. $1/N$, $p = 0.327$ vs. TC-MinVar, and $p = 0.130$ vs. Finite-Regime). The block-bootstrap analysis therefore provides no evidence that the observed Sharpe-ratio differences are statistically distinguishable from zero over the evaluation sample. This is an absence of evidence for a difference, not evidence that the strategies perform equally. As shown in Figure \ref{fig:bootstrap}, the bootstrap distribution of $\Delta \text{SR}$ relative to Nominal CVaR is approximately symmetric and centered near the observed estimate $-0.071$. The bootstrap confidence intervals provide an exploratory gauge of estimation uncertainty, with the null value $\Delta \text{SR} = 0$ comfortably contained within the 95\% confidence interval in all four comparisons.
...
Two distinct tolerances appear in this work and should not be confused. The solver feasibility and optimality tolerance $10^{-8}$ is internal to each individual LP or QP solve. The exchange termination tolerance $\varepsilon = 10^{-4}$ governs when the outer constraint-generation loop stops, that is when the grid-restricted gap $\widehat G_k - \mathrm{LB}_k$ is deemed closed. The former controls the accuracy of a single subproblem; the latter controls the accuracy of the overall grid-restricted optimum.
...
\subsection{Bandwidth orientation}\label{sec:bwgeom}
Table \ref{tab:sens_bw} varies the scale of the bandwidth but not its orientation: the baseline rule is diagonal, $H_{\mathrm{diag}} = T^{-1/3}\operatorname{diag}(\hat{\sigma}^2_{v}, \hat{\sigma}^2_{d})$, even though log-VIX and drawdown are correlated, with a mean within-window correlation of 0.70 over the 377 windows. A diagonal bandwidth orients the kernel along the coordinate axes and therefore averages over states that the joint distribution makes implausible. We repeat the full rolling experiment with the full-covariance rule $H_{\mathrm{full}} = T^{-1/3}\hat{\Sigma}_y$, which carries the same scaling and adds only the off-diagonal term.

\begin{table}[!htbp]
\centering
\caption{Diagonal versus full-covariance bandwidth, on the same 377-window calendar as Table \ref{tab:performance}}
\label{tab:bwgeom}
\small
\begin{tabular}{lrrrrrr}
\toprule
\textbf{Bandwidth} & \textbf{Sharpe} & \makecell[r]{\textbf{Turn.}\\\textbf{(\%)}} & \textbf{Wealth} & \makecell[r]{\textbf{Worst}\\\textbf{CVaR (\%)}} & \makecell[r]{\textbf{Active}\\\textbf{states}} & \makecell[r]{\textbf{Mean}\\\textbf{ESS}} \\
\midrule
$H_{\mathrm{diag}}$ (baseline) & 0.809 & 9.48 & 25.04 & 4.525 & 3.66 & 83.0 \\
$H_{\mathrm{full}}$ & 0.810 & 9.35 & 25.40 & 4.535 & 3.63 & 64.3 \\
\bottomrule
\end{tabular}
\end{table}

Table \ref{tab:bwgeom} shows that orientation does not matter here. The Sharpe ratio moves from 0.809 to 0.810, the mean worst-case conditional CVaR from 4.525\% to 4.535\%, the mean number of active states from 3.66 to 3.63 and turnover from 9.48\% to 9.35\%. The one quantity that responds is the effective sample size at the active states, which falls from 83.0 to 64.3: aligning the kernel with the principal axis of the state cloud concentrates the weights on fewer observations, as expected. That this sharpening leaves the allocation and the worst-case value essentially unchanged is itself informative, since it indicates that the binding states are determined by the geometry of the state domain rather than by the fine structure of the local weighting. The diagonal rule is retained for the baseline because it is the simpler of the two and nothing in this comparison favours the alternative.
...
\subsection{Sensitivity to the state-domain expansion}
\label{sec:margin}
Because the robustness operator takes a worst case over $\mathcal{U}$, and
$\mathcal{U}$ is the training-window box expanded by a margin $\delta$ beyond the
observed extrema, that margin is not a minor implementation setting: it
determines which states count as adverse. We therefore repeat the complete
rolling experiment for $\delta \in \{0\%, 5\%, 10\%, 20\%\}$, where $10\%$
reproduces the baseline. Estimation window, holding period, weight cap, return
target, bandwidth rule and transaction costs are identical across the four runs,
so the margin is the only quantity that varies. This separates robustness
attributable to the continuous-state formulation from robustness induced by the
particular choice of domain size.

\begin{table}[!htbp]
\centering
\caption{Sensitivity to the state-domain expansion margin $\delta$. All four runs
use the identical 377-window calendar; the last column is the fraction of windows
in which at least one generated state lies on the boundary of the expanded box}
\label{tab:margin}
\footnotesize
\begin{adjustbox}{max width=\textwidth}
\begin{tabular}{lrrrrrrrrr}
\toprule
\makecell[l]{\textbf{Margin}\\\textbf{$\delta$}} & \makecell[r]{\textbf{Ret.}\\\textbf{(\%)}} & \makecell[r]{\textbf{Vol.}\\\textbf{(\%)}} & \textbf{Sharpe} & \makecell[r]{\textbf{Max DD}\\\textbf{(\%)}} & \makecell[r]{\textbf{Turn.}\\\textbf{(\%)}} & \textbf{Wealth} & \makecell[r]{\textbf{Mean}\\\textbf{ESS}} & \makecell[r]{\textbf{Active}\\\textbf{states}} & \makecell[r]{\textbf{Boundary}\\\textbf{(\%)}} \\
\midrule
$0\%$  & 11.33 & 13.96 & 0.811 & $-41.77$ & 9.51 & 25.40 & 83.3 & 3.75 & 48.8 \\
$5\%$  & 11.35 & 13.96 & 0.813 & $-41.71$ & 9.51 & 25.63 & 84.5 & 3.62 & 48.5 \\
$10\%$ & 11.28 & 13.95 & 0.809 & $-41.69$ & 9.48 & 25.04 & 83.0 & 3.66 & 49.3 \\
$20\%$ & 11.33 & 13.94 & 0.813 & $-41.45$ & 9.20 & 25.44 & 82.9 & 3.62 & 49.1 \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}

Table \ref{tab:margin} shows that the reported conclusions are insensitive to
this choice. Across a fourfold change in the margin, and including the case
$\delta = 0$ in which the domain is exactly the observed training-window box, the
Sharpe ratio varies by 0.005, terminal wealth by less than \$0.60, the average
number of generated states by 0.13, and the average worst-case CVaR by 0.003
percentage points. The baseline $\delta = 10\%$ is therefore not what produces
the results.

The last column is more interesting. Boundary activity is frequent but not
universal: in roughly 49\% of the rolling windows at least one generated stress
state lies on the boundary of the expanded box, and that proportion is
essentially unchanged as the boundary is moved outward. This suggests that
conditional CVaR often increases towards the edge of the admissible state space,
while interior worst-case states remain relevant in the remaining windows.
Enlarging the margin moves the boundary without changing this structure, which
explains why the performance statistics barely respond. It also
sharpens the support question of Section \ref{sec:sensitivity}: the states that
drive the robust constraint are, by construction, those furthest from the data
cloud, and therefore those estimated with the least local support.

\subsection{Sensitivity to the expected-return target}\label{sec:target}
The baseline sets the return constraint to the cross-sectional median of the estimated annualized asset means. Section \ref{sec:results} attributes the elevated volatility of Robust SIP to the interaction of return targeting, state conditioning and boundary effects, without separating them. This experiment separates them. We repeat the full rolling experiment with the target at the 25th, 50th and 75th percentiles of the estimated means, and once with the constraint set to the minimum attainable portfolio mean so that it is present but never binding. Everything else is held at the baseline.

\begin{table}[!htbp]
\centering
\caption{Sensitivity to the expected-return target. $Q_p$ denotes the $p$th percentile of the estimated annualized asset means and $Q_{50}$ is the baseline; the last column is the mean number of active states, on the same 377-window calendar as Table \ref{tab:performance}}
\label{tab:target}
\footnotesize
\begin{adjustbox}{max width=\textwidth}
\begin{tabular}{lrrrrrrrr}
\toprule
\textbf{Target} & \makecell[r]{\textbf{Ret.}\\\textbf{(\%)}} & \makecell[r]{\textbf{Vol.}\\\textbf{(\%)}} & \textbf{Sharpe} & \makecell[r]{\textbf{Max DD}\\\textbf{(\%)}} & \makecell[r]{\textbf{Turn.}\\\textbf{(\%)}} & \textbf{Wealth} & $\boldsymbol{N_{\text{eff}}}$ & \textbf{Active} \\
\midrule
Non-binding & 10.84 & 14.00 & 0.774 & $-43.57$ & 6.12 & 21.80 & 7.27 & 3.79 \\
$Q_{25}$ & 10.91 & 14.02 & 0.778 & $-43.57$ & 6.38 & 22.25 & 7.29 & 3.80 \\
$Q_{50}$ & 11.28 & 13.95 & 0.809 & $-41.69$ & 9.48 & 25.04 & 7.36 & 3.66 \\
$Q_{75}$ & 13.07 & 14.54 & 0.899 & $-39.49$ & 13.64 & 42.62 & 7.36 & 3.15 \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}

Table \ref{tab:target} reports the four specifications. Its $Q_{50}$ row reproduces the baseline of Table \ref{tab:performance} exactly, which serves as a check on the experiment. The first conclusion is negative and clear: the return target is not what makes Robust SIP volatile. Removing the constraint altogether leaves annualized volatility at 14.00\% against 13.95\% at the baseline, and across all four specifications volatility varies only between 13.95\% and 14.54\%. The elevated volatility relative to the CVaR benchmarks, which run near 12.2--12.4\%, is a property of minimizing a worst-case conditional CVaR over a state domain, not a by-product of return targeting. Concentration behaves the same way, with $N_{\text{eff}}$ confined to 7.27--7.36.

The second conclusion is more consequential. The target is a first-order driver of everything else. Raising it from $Q_{50}$ to $Q_{75}$ raises the Sharpe ratio from 0.809 to 0.899, terminal wealth from \$25.04 to \$42.62, and shallows the maximum drawdown from $-41.7\%$ to $-39.5\%$, at the cost of raising turnover from 9.48\% to 13.64\%. Within the Robust SIP specification, the Sharpe ratio is highly sensitive to the expected-return target, increasing from 0.809 at $Q_{50}$ to 0.899 at $Q_{75}$. Because the benchmark strategies are not re-estimated under the alternative target levels, this sensitivity analysis should not be interpreted as establishing cross-strategy parity at $Q_{75}$. Instead, it shows that part of the observed performance profile depends materially on the return-target convention.

We do not adopt $Q_{75}$ as the baseline, and we caution against reading this row as a recommendation. The four targets are compared after observing the full out-of-sample record, so selecting the best-performing one would be an in-sample choice presented as an out-of-sample result. What the experiment establishes is narrower and, for the purposes of this paper, more useful: a reader comparing Table \ref{tab:performance} across strategies is also, implicitly, comparing a particular return-target convention, and the sensitivity of the outcome to that convention is larger than the differences between the strategies themselves. The interaction between the return constraint and the robust layer also runs in the expected direction: the mean number of active states falls from 3.79 to 3.15 as the target tightens, and the mean worst-case conditional CVaR rises from 4.46\% to 4.94\%, since a tighter return floor forces the allocation into states with less favourable tails.
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\figwidth]{Fig14.pdf}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\figwidth]{Fig13.pdf}
...
\subsection{A regularized conditional comparator}\label{sec:shrinkage}
The state-conditioned specification of Section \ref{sec:referee_benchmarks} conditions on the single observed state $y_T$ and nothing else, and it is fair to ask whether it is too fragile a comparator to be informative. We therefore embed it in a one-parameter family that shrinks the conditional law towards the unconditional one,
\begin{equation}
\hat{P}^{(\lambda)}_t = (1-\lambda)\,\hat{P} + \lambda\,\hat{P}_{y_T}, \qquad \lambda \in [0,1],
\label{eq:shrinkage}
\end{equation}
and solves nominal CVaR under $\hat{P}^{(\lambda)}_t$. Here $\hat{P}$ places equal mass on the training sample and $\hat{P}_{y_T}$ carries the Nadaraya--Watson weights at the current state, so $\lambda$ interpolates continuously between the two benchmarks already in the study. The rolling calendar and every estimation setting are those of the baseline.

\begin{table}[!htbp]
\centering
\caption{Nominal CVaR under the shrunk conditional law of Equation \eqref{eq:shrinkage}, on the same 377-window calendar as Table \ref{tab:performance}}
\label{tab:shrinkage}
\small
\begin{tabular}{lrrrrrr}
\toprule
$\boldsymbol{\lambda}$ & \makecell[r]{\textbf{Ret.}\\\textbf{(\%)}} & \makecell[r]{\textbf{Vol.}\\\textbf{(\%)}} & \textbf{Sharpe} & \makecell[r]{\textbf{Max DD}\\\textbf{(\%)}} & \makecell[r]{\textbf{Turn.}\\\textbf{(\%)}} & \textbf{Wealth} \\
\midrule
0.00 & 10.76 & 12.23 & 0.880 & $-35.76$ & 8.23 & 22.92 \\
0.25 & 10.44 & 12.38 & 0.843 & $-38.73$ & 16.34 & 20.57 \\
0.50 & 10.23 & 12.53 & 0.816 & $-41.67$ & 21.83 & 19.17 \\
0.75 & 10.40 & 12.82 & 0.811 & $-41.76$ & 28.05 & 20.03 \\
1.00 & 10.05 & 13.01 & 0.773 & $-42.73$ & 35.74 & 17.82 \\
\bottomrule
\end{tabular}
\end{table}

Table \ref{tab:shrinkage} reports five values of $\lambda$. Two endpoints serve as an implementation check: $\lambda = 0$ reproduces the Nominal CVaR row of Table \ref{tab:performance} exactly, and $\lambda = 1$ reproduces the state-conditioned row of Table \ref{tab:referee_benchmarks} exactly. Between them, every reported quantity degrades monotonically in $\lambda$. The Sharpe ratio falls from 0.880 to 0.773, the maximum drawdown deepens from $-35.8\%$ to $-42.7\%$, and average turnover rises from 8.2\% to 35.7\%, a factor of 4.3. No interior value of $\lambda$ recovers what conditioning costs; regularizing the conditional law slows the deterioration without reversing it. The comparison in Section \ref{sec:referee_benchmarks} is therefore not against a straw man, but against the best member of a family that contains it.

The diagnostic is not a small-sample artifact. The mean effective sample size at the conditioning state is 198.3 observations out of 1260, so the conditional distribution is estimated from a substantial fraction of the window. What destabilizes the allocation is not the precision of $\hat{P}_{y_T}$ at any one date but its movement between dates, and shrinking towards $\hat{P}$ damps that movement only in proportion to $1-\lambda$.

Read against the baseline, the family also locates the contribution of the robust layer. Robust SIP attains a Sharpe ratio of 0.809, which sits between the $\lambda = 0.5$ and $\lambda = 0.75$ members, but it does so at an average turnover of 9.48\%, close to the unconditional 8.2\% and roughly a third of what those two members require, while accumulating more terminal wealth (\$25.04) than any member of the family. Taking a worst case over a state domain and shrinking towards the unconditional law are thus not two routes to the same place: the first buys state-awareness at almost unconditional trading cost, the second pays for it in turnover. This does not overturn the central finding of Section \ref{sec:results}, since the unconditional $\lambda = 0$ specification still records the highest Sharpe ratio in the table.

\subsection{Sensitivity to the risk-free rate}\label{sec:riskfree}
Every Sharpe ratio reported so far uses a zero risk-free rate, which is difficult to defend over a sample running from 1994 to 2026. All five strategies are fully invested, so a constant rate is subtracted from every mean return alike; the ordering can nonetheless change, because a fixed subtraction penalizes a low-volatility strategy more than a high-volatility one. Table \ref{tab:riskfree} recomputes the Sharpe ratios at 0, 2 and 4 percent per annum.

\begin{table}[!htbp]
\centering
\caption{Out-of-sample Sharpe ratios under a constant annual risk-free rate}
\label{tab:riskfree}
\small
\begin{tabular}{lrrrrr}
\toprule
\textbf{Strategy} & \makecell[r]{\textbf{Ret.}\\\textbf{(\%)}} & \makecell[r]{\textbf{Vol.}\\\textbf{(\%)}} & \makecell[r]{\textbf{Sharpe}\\$r_f=0\%$} & \makecell[r]{\textbf{Sharpe}\\$r_f=2\%$} & \makecell[r]{\textbf{Sharpe}\\$r_f=4\%$} \\
\midrule
1/N & 12.24 & 17.00 & 0.720 & 0.602 & 0.485 \\
TC-MinVar & 10.73 & 12.26 & 0.875 & 0.712 & 0.549 \\
Nominal CVaR & 10.76 & 12.23 & 0.880 & 0.717 & 0.553 \\
Finite-Regime CVaR & 11.15 & 12.38 & 0.901 & 0.739 & 0.577 \\
Robust SIP & 11.28 & 13.95 & 0.809 & 0.665 & 0.522 \\
\bottomrule
\end{tabular}
\end{table}

The ordering is identical at all three rates, so the conclusions of Section \ref{sec:results} do not depend on the zero-rate convention. The one effect worth noting runs mildly in favour of the proposed method: because Robust SIP carries the highest volatility among the optimized strategies, its Sharpe ratio declines most slowly as the rate rises, and its shortfall against the best benchmark narrows from 0.092 at $r_f = 0\%$ to 0.055 at $r_f = 4\%$. This remains a shortfall, and Section \ref{sec:computational} has already shown that differences of this size are not statistically distinguishable from zero. We report a constant rate rather than a realized Treasury-bill series, so this is a check that the ranking survives a non-zero rate, not a measurement of realized excess returns.
...
The proposed state-robust CVaR framework provides a principled and interpretable method for incorporating continuous market covariates into portfolio optimization without imposing artificial discrete regime boundaries. In the reported approximately 31.5-year empirical experiment on US industry portfolios, the continuous-state strategy achieved greater terminal wealth (\$25.04) than TC-MinVar (\$22.65) and nominal CVaR (\$22.92), but lower wealth than naive $1/N$ (\$29.25) and finite-regime CVaR (\$25.68). It also exhibited lower Sharpe ratios and deeper maximum drawdowns than the nominal and finite-regime CVaR benchmarks, and paired circular block percentile bootstrap testing does not provide sufficient evidence to distinguish these Sharpe-ratio differences from zero at conventional significance levels. Thus, the empirical findings do not establish superior risk-adjusted or crisis performance. Two specification experiments qualify how this shortfall should be read. It is not produced by the return-target convention in the way one might assume: removing the return constraint entirely leaves annualized volatility unchanged at 14.00\%, so the elevated volatility is a property of worst-case conditional CVaR minimization rather than of return targeting. The Sharpe ratio, by contrast, is highly sensitive to that convention, rising to 0.899 under a 75th-percentile target. Since the benchmarks are not re-estimated at that target, this does not establish parity with them; it establishes that the return-target convention accounts for a material part of the performance profile, and that a comparison across strategies at one fixed convention is correspondingly less decisive than it appears.

The results do not establish that continuous-state robustification dominates nominal or finite-regime CVaR in risk-adjusted performance. The evidence supports a more specific conclusion. First, continuous state dependence can be incorporated into a convex robust portfolio model without requiring discrete regime boundaries. Second, despite the continuum of state-indexed constraints, the finite grid approximation is computationally sparse, and sparse in a way that does not degrade with resolution: an average of 3.66 out of 441 candidate blocks is generated at the baseline, and in a matched comparison against the extensive-form dense LP over 40 windows the average stays between 3.27 and 3.42 as the candidate set grows eightfold from 121 to 961 states, while dense solution time grows by a factor of 17.1 and exchange time by a factor of 3.5. It is this stability, rather than the speedup on any one instance, that makes a fine discretization of a continuous state space affordable. Third, robustness across the state domain substantially stabilizes the highly reactive allocations produced by pure state-conditioned CVaR, which turns over 35.7\% of the portfolio per rebalancing against 9.48\% for the robust formulation, and no intermediate degree of shrinkage towards the unconditional law recovers the difference. The principal contribution is therefore methodological and computational, with state robustness acting as a regularizing mechanism rather than as a guarantee of superior investment performance.
...
\textbf{Data Availability:} The Kenneth French 30 Industry Portfolio returns \cite{french2026data} and CBOE VIX observations \cite{cboe2026vix} used in this study were obtained from their publicly accessible source pages. Source URLs, retrieval information, preprocessing code, and the aligned research dataset are documented in the accompanying repository, subject to the original providers' terms of use. Reference \cite{bezoui2026robust} cites the Zenodo concept DOI, which always resolves to the most recent archived version; the version-specific DOI and the Git commit identifier of the snapshot corresponding to the results reported here are recorded in the release notes of that version. The GitHub repository may contain later development versions \cite{bezoui2026robust}.
