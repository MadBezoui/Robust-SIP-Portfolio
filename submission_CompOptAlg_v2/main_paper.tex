...
\subsection{Solution stability and turnover regularization}
Rolling re-estimation coupled with the near-flatness of the robust worst-state CVaR objective leads to allocation drift. Successive optimal allocations $w^*$ can shift significantly despite producing near-identical objective values. To mitigate the resulting high turnover and transaction cost drag, a practical remedy is to introduce a proximity regularizer $\lambda \|w - w_{\mathrm{prev}}\|_1$ directly into the master LP. Linearized via auxiliary variables, this penalty suppresses turnover during volatile regime transitions without compromising the state-robust properties.
