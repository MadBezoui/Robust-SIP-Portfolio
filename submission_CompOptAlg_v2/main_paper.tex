...
\end{equation}
Since we do not explicitly compute $L_\Phi$, we rely on grid-refinement sensitivity to assess the sensitivity of the grid-restricted approximation.
...
$$
where $\nabla_\theta p_t(\theta) = p_t(\theta) H^{-1}(\bar{y}(\theta) - y_{t-1})$. This permits the derivation of a computable uniform Lipschitz constant $L_\Phi \le \frac{4 R M_L}{\tau \lambda_{\min}(H)}$. 
Consequently, the grid separation acts as an \emph{inexact oracle with additive inexactness} $\delta = L_\Phi \rho$ (where $\rho$ is the grid dispersion). Following the error-propagation results of Oustry and Cerulli (2025), the exchange algorithm converges to a solution whose continuous suboptimality is bounded by $\varepsilon + L_\Phi \rho$. To verify this bound in practice, a posteriori projected gradient ascent confirms that the true continuous supremum deviates from the grid supremum by less than $10^{-4}$.
