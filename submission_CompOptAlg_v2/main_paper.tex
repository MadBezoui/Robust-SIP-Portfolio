...
Because the portfolio allocation $w^k$ is fixed during separation, the conditional CVaR values $\Phi_\tau(w^k,\theta)$ for all $\theta\in\widehat{\mathcal{U}}$ are evaluated in overall $O(T\log T+KT)$ time by sorting the $T$ portfolio losses only once in $O(T\log T)$, followed by a sequential $O(T)$ summation over the grid's $K$ states to aggregate the reordered kernel probabilities.
\begin{equation}
...
Consequently, the grid separation acts as an \emph{inexact oracle with additive inexactness} $\delta = L_\Phi \rho$ (where $\rho$ is the grid dispersion). Following the error-propagation results of Oustry and Cerulli (2025, Theorem 3.1), the exchange algorithm converges to a solution whose continuous suboptimality is bounded by $\varepsilon + L_\Phi \rho$. We validate the analytical gradient $\nabla_\theta \Phi_\tau$ using central finite differences, confirming numerical agreement to within $10^{-7}$ precision. To verify the continuous optimality bound in practice, we conduct an a posteriori projected gradient ascent initialized from the worst grid state. Table \ref{tab:gap_verification} reports the theoretical Lipschitz bound $L_\Phi \rho$ and the empirically verified continuous suboptimality gap for a sample of testing windows. Across all periods, the true continuous supremum deviates from the grid supremum by less than $10^{-4}$, confirming the practical adequacy of the $21\times21$ grid.
...
\begin{tabular}{lrrrr}
\toprule
\textbf{Testing Window Ends} & \textbf{Lipschitz Const. $L_\Phi$} & \textbf{Grid Disp. $\rho$} & \textbf{Bound $L_\Phi \rho$} & \textbf{Verified Gap} \\
\midrule
...
Figure 11 illustrates convergence for one selected rolling window. Table \ref{tab:computational} reports a single representative-window snapshot used specifically for the dense-grid benchmarking, as running the exact dense $41\times41$ LP over all 377 windows proved computationally prohibitive; therefore, the objective values and numbers of active states in Figure 11 and Table \ref{tab:computational} are not expected to coincide. For the selected benchmark window and the reported extensive-form implementation, the dense $41\times41$ LP did not return a primal solution within the 600-second solver limit.
