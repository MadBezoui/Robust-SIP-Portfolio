...
include(joinpath(@__DIR__, "01_run_backtest.jl"))
...
function run_all_sensitivities()
    ...
    for (s_idx, t_start) in enumerate(subset_indices)
        ...
        for g in reverse(grid_sizes) # Do 81 first for baseline
            ...
            t0 = time()
            w_rob, lb, ub, active_thetas, _, _, _, _, _ = solve_robust_sip(X_train, Y_train, grid_thetas, H, mu_train ./ 252.0, tau, median(mu_train) / 252.0; max_iter=15, tol=1e-4, max_weight=max_weight)
            rt = time() - t0
            rt_sums[g] += rt
    ...
    ts_path = joinpath(output_dir, "strategy_holding_period_returns.csv")
    if isfile(ts_path)
