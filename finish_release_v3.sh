...
pdflatex -interaction=batchmode main_paper.tex
bibtex main_paper
pdflatex -interaction=batchmode main_paper.tex
pdflatex -interaction=batchmode main_paper.tex
cd ..
...
git add .
git commit -m "Final exact reproducibility fixes (run full pipeline, Table 6, 11, ESS, Eq 35, Bootstrap Fix) for v1.6.0"
